...
static int video_framebufferInit(void)
{
	...
	/* Virtual height = 3x the (config-forced) 1080p physical so the firmware allocates a
	 * triple-buffer-tall framebuffer in this SINGLE alloc — together with depth + pixel-order
	 * below. Doing it here (not via a second SET_VIRTUAL/ALLOCATE_BUFFER call afterwards) keeps
	 * the surface's format intact: a follow-up reallocation perturbed it and corrupted all the
	 * world colours. The renderer page-flips between the 3 stacked buffers; it degrades to
	 * double/single if the firmware grants less. */
	video_mailbox[7] = tag_setvirtwh;
	video_mailbox[8] = 8u;
	video_mailbox[9] = 8u;
	video_mailbox[10] = PLO_RPI_FB_WIDTH;
	video_mailbox[11] = 3u * 1080u;   /* 3240 = 3 x 1080 (config.txt forces hdmi 1080p) */
	...
	/* Report the PHYSICAL (displayed) size from the SET_PHYSICAL response ([5],[6]) — NOT the
	 * virtual size ([10],[11]) which is now 3x tall for triple-buffer. graphmode/fbcon/rpi4-fb
	 * see one 1080p buffer (buffer 0); the renderer derives buffers 1/2 at +pitch*height and
	 * probes the granted virtual height. */
	video_common.width = video_mailbox[5];
	video_common.height = video_mailbox[6];
	video_common.pitch = video_mailbox[33];
	video_common.bpp = video_mailbox[20];
	video_common.size = video_common.pitch * video_common.height;   /* buffer 0 only */
